...
void dictation_session_callback(DictationSession *session, DictationSessionStatus status, 
  ...
  if(status == DictationSessionStatusSuccess) {
    ...
    snprintf(s_last_text, sizeof(s_last_text), "Add Task ?:\n\n%s", transcription);
...
void add_task_handler(ClickRecognizerRef recognizer, void *context) {
...
static void click_config_provider(void *context) {
  window_single_click_subscribe(BUTTON_ID_SELECT, add_task_handler);
}
...
static void window_load(Window *window) {
  ...
  text_layer_set_text(s_output_layer, "Press Select to add task!");
